...
struct System {
    ...
    constexpr bool is_zero_or_one(const Term &term) const noexcept {
        if (term == TERM_ZERO) { return true; }
        if (term.p_index) {
            if (p.get(term.p_index - 1) == VAR::UNKNOWN) { return false; }
        }
        if (term.q_index) {
            if (q.get(term.q_index - 1) == VAR::UNKNOWN) { return false; }
        }
        return true;
    }
    ...
    constexpr bool simplify() noexcept {
        ...
        // Phase 2: Propagate right-hand sides to left-hand sides.
        ...
        // Phase 3: Solve subsystems of the following form:
        //     a + b == 0 or 1
        //     a * b == 0 or 1
